      ...
      <v-col cols="12" md="9">
        <v-card class="content blue-shadow">
          ...
          <div class="map-container">
            ...
            >
              <Marker
                v-for="event in filteredEventsWithLocation"
                :key="event.id"
                :options="{
          ...
          <!-- Category Carousels -->
          <div v-for="category in categoriesWithEvents" :key="category" class="category-section">
            <h2 class="category-header">{{ category }}</h2>
            <div class="carousel-wrapper">
              <div class="carousel-row">
                <div
                  v-for="event in getEventsByCategory(category)"
                  :key="event.id"
                  class="carousel-card"
                  tabindex="0"
                  @click="goToEventDetail(event.id)"
                ...
                >
                  <img v-if="event.imageUrl" :src="event.imageUrl" class="carousel-image" />
                  <div class="carousel-title">{{ event.eventName }}</div>
                  <div class="carousel-subtitle">Owner: {{ event.ownerName || "Unknown" }}</div>
                  <div class="carousel-subtitle">People attending: {{ event.AttendanceCount }}</div>
                </div>
              </div>
            </div>
          </div>
  ...
  computed: {
    ...
    categoriesWithEvents() {
      return this.categories.filter((category) =>
        this.filteredEvents.some((event) =>
