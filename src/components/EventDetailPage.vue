...
.message {
  ...
  font-size: 16px;
   ...
   <template>
    <div class="container">
      <div v-if="event" class="box">
        <img v-if="imageUrl" :src="imageUrl" alt="Event Image" class="event-image" />
        <h1>{{ event.eventName }}</h1>
        <p><strong>Owner:</strong> {{ ownerName }}</p>
        <p><strong>Date:</strong> {{ event.eventDate }}</p>
        <p><strong>Description:</strong> {{ event.eventDescription }}</p>
...
export default {
  data() {
    return {
      ...
      ownerName: "Loading...",
      message: "",
      imageUrl: null, // To store the event's image URL
    };
  },
  async created() {
    const eventId = this.$route.params.id; // Get the event ID from the URL
    ...
    try {
      ...
      if (eventDoc.exists()) {
        ...
        if (this.event.imagePath) {
          const imageRef = ref(storage, this.event.imagePath);
